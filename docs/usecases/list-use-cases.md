...
5. **Resolución explícita de conflicto `409` (no silenciosa):** ante conflicto de versión, la web debe mostrar diálogo de resolución con exactamente dos opciones de usuario:
   - **Mantener local:** conservar borrador local y reintentar sincronización usando la nueva base remota.
   - **Mantener remoto:** rehidratar `LOCAL_DRAFT` con snapshot remoto y descartar el pendiente local.
6. **Refresh cross-tab por `storage` event:** en dos pestañas del mismo navegador, los cambios en `lists.localDraft` deben refrescar la vista de la otra pestaña solo cuando no hay cambios locales pendientes; si hay pendientes, se marca conflicto remoto sin pisar edición local.
7. **Regla de overwrite server -> local (solo recuperación explícita):** los datos remotos solo pueden sobrescribir `LOCAL_DRAFT` en bootstrap/login recovery o tras una acción explícita de recuperación del usuario (incluida resolución de `409`).
8. **Consistencia con endpoints:** ningún criterio debe exigir endpoints retirados o renombrados (ej.: `PATCH /status`, `POST /duplicate`), y todas las reglas anteriores se validan sobre endpoints vigentes (`/activate`, `/reuse`, `/finish-edit`, `/autosave`).
...
| DRAFT en dos pestañas web | `storage` event con cambio de `lists.localDraft` | DRAFT | Si no hay cambios locales pendientes: refresca draft local. Si hay pendientes: no sobreescribe local y marca cambios remotos disponibles | Sin mutación directa en servidor (solo sincronización posterior según decisión del usuario) | Sin cambios en listas `ACTIVE`/`COMPLETED` | Refresh pasivo cuando es seguro; aviso de cambios remotos cuando hay edición local pendiente |
| DRAFT en dos pestañas web | `PUT /api/lists/autosave` devuelve `409` por versión | DRAFT (con conflicto pendiente) | Se conserva `LOCAL_DRAFT` hasta decisión explícita; no hay overwrite automático | Sin overwrite silencioso del remoto; se expone `remoteUpdatedAt` para resolver | Sin cambios en listas `ACTIVE`/`COMPLETED` | Diálogo obligatorio con dos opciones: **Mantener local** o **Mantener remoto** |
